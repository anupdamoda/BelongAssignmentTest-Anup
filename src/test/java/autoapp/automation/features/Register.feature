Feature: Registering to automation practice
  ...
  Scenario: Validate user is able to see error message if email address is incorrect
    Given Automation practice application is opened
    When I click sign in
    And I create account with emailid "testRandom"
    Then I should be able to see error message "Invalid email address."